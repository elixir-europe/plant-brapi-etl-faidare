...
data_0 = {"refIds": [1, 2, 3, '4'], "foo": [1, 2, 3], "genus": "Zea", "species": "mays", "falseField": False}
...
class TestResolve(unittest.TestCase):
    def test_resolve(self):
        ...
        expected = [data_1, data_2, data_3, data_4]
    ...
    def test_resolve_join3(self):
        input = {"{join}": ["foo", "{.foo}", ["foo", "{.foo}"]]}
        actual = resolve(input, data_0, data_index)
        expected = "foo123foo123"
        self.assertEqual(actual, expected)

    def test_resolve_if1(self):
        input = {"{if}": "foo", "{then}": "then"}
        actual = resolve(input, data_0, data_index)
        expected = "then"
        self.assertEqual(actual, expected)

    def test_resolve_if2(self):
        input = {"{if}": "{.nonExistingField}", "{then}": "then"}
        actual = resolve(input, data_0, data_index)
        expected = None
        self.assertEqual(actual, expected)
    ...
    def test_resolve_if4(self):
        input = {"{if}": "{.nonExistingField}", "{then}": "bar", "{else}": "else"}
        actual = resolve(input, data_0, data_index)
        expected = "else"
        self.assertEqual(actual, expected)

    def test_resolve_if5(self):
        input = {"{if}": "{.falseField}", "{then}": "bar", "{else}": "else"}
        actual = resolve(input, data_0, data_index)
        expected = "else"
        self.assertEqual(actual, expected)
    ...
    def test_resolve_or1(self):
        input = {"{or}": ["foo", "bar", "baz"]}
        actual = resolve(input, data_0, data_index)
        expected = "foo"
        self.assertEqual(actual, expected)

    def test_resolve_or2(self):
        input = {"{or}": ["{.falseField}", "{.nonExistingField}", "baz"]}
        actual = resolve(input, data_0, data_index)
        expected = "baz"
        self.assertEqual(actual, expected)
