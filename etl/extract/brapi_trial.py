...
def list_object(options):
    ...
    """
    source, logger, entity, trial_db_id = options
...
def fetch_trial(source, logger, entities, pool, trial_db_id):
    """
...
def extract_source(source, entities, config, output_dir):
    ...
    try:
        ...
        fetch_trial(source, logger, entities, pool, config['options']['trialDbId'])
        ...
        # Link entities from second level details
        fetch_all_links(source, logger, entities)

        # Detail entities (for object that might have been discovered by links)
        fetch_all_details(source, logger, entities, pool)

        # Link entities from third level details
        fetch_all_links(source, logger, entities)
