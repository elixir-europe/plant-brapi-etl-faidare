...
def dump_documents_in_bulk_files(source, documents, source_bulk_dir, logger):
    ...
    json_store.close()
    logger.info("Saved {} documents in {} bulk files.".format(document_count, json_store.file_index))


def validate(documents, validation_config, logger):
    """
    Iterate on all documents and validate its schema (if needed)
    """
